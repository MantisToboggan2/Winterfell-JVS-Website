export async function onRequest(context) {
    ...
    if (request.method === "POST") {
        try {
            ...
            if (player_count > stats.peak) {
                stats.peak = player_count;
            }
    ...
    const data = await env.STATS_DB.get("winterfell_stats", { type: "json" }) || { live: 0, map: "N/A" };
    
    const responseData = {
        ...data,
        avg: data.total_checks > 0 ? Math.round(data.total_sum / data.total_checks) : 0,
